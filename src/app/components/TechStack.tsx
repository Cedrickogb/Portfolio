      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <h1 className="font-display text-4xl md:text-5xl text-gray-900 dark:text-white mb-4 tracking-wider drop-shadow-[0_0_10px_rgba(132,204,22,0.5)] transition-colors">
            <span className="text-primary-lime">TECH</span> COLLECTION
          </h1>
          <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto font-mono text-sm border-l-2 border-primary-lime pl-4 text-left md:text-center md:border-l-0 md:border-b-2 md:pb-2 md:pl-0 inline-block transition-colors">
            ...
            {Object.values(TECH_DATA).map((tech, index) => (
              ...
              >
                ...
                <div className="h-32 flex items-center justify-center bg-gray-50 dark:bg-[#181a1b] rounded-lg mb-2 relative overflow-hidden group-hover:bg-opacity-80 transition-colors border border-gray-100 dark:border-none">
                  {tech.isTextIcon ? (
                     <span className={`font-display text-3xl font-bold text-gray-400 dark:text-gray-400 group-hover:${tech.color.replace('text-', 'text-')} transition-all duration-300`}>
                        {tech.textSymbol}
                     </span>
                  ) : (
                    <i className={`${tech.iconClass} text-5xl text-gray-400 dark:text-gray-400 group-hover:${tech.color.replace('text-', 'text-')} transition-all duration-300 transform group-hover:scale-110`}></i>
                  )}
                  {/* Subtle gradient only in dark mode or lighter in light mode */}
                  <div className="absolute inset-0 bg-gradient-to-t from-white/0 dark:from-black/60 to-transparent"></div>
                </div>
                <div className="px-2 pb-2">
                  <div className="flex justify-between items-center mb-1">
                    <h3 className={`font-display text-[10px] md:text-xs text-gray-800 dark:text-gray-200 group-hover:${tech.color.replace('text-', 'text-')}`}>
                        {tech.name}
                    </h3>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-800 h-1.5 rounded-full overflow-hidden transition-colors">
                    <div className={`${tech.color.replace('text-', 'bg-')} h-full w-[90%]`}></div>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="sticky top-24 bg-[#dc0a2d] p-4 rounded-3xl border-4 border-[#89061d] shadow-[10px_10px_0px_rgba(0,0,0,0.2)] dark:shadow-[10px_10px_0px_rgba(0,0,0,0.5)]">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="relative w-24 h-24 mx-auto">
                    ...
                    <div className="absolute top-1 left-[42%] text-gray-600 text-[10px]"><i className="fas fa-caret-up"></i></div>
                    <div className="absolute bottom-1 left-[42%] text-gray-600 text-[10px]"><i className="fas fa-caret-down"></i></div>
                    <div className="absolute top-[42%] left-1 text-gray-600 text-[10px]"><i className="fas fa-caret-left"></i></div>
                    <div className="absolute top-[42%] right-1 text-gray-600 text-[10px]"><i className="fas fa-caret-right"></i></div>
                </div>
